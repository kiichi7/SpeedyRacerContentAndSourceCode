// Project: SpeedyRacer, File: UIRenderer.cs
// Namespace: SpeedyRacer.Graphics, Class: UIRenderer
...
namespace SpeedyRacer.Graphics
{
	...
	public class UIRenderer : IDisposable
	{
		...
		private Vector3 carPos = SpeedyRacerManager.Player.CarPosition;
		...
		public void RenderMenuTrackBackground()
		{
			...
			if (carMenuTime > SpeedyRacerManager.Landscape.BestReplay.LapTime)
				carMenuTime -= SpeedyRacerManager.Landscape.BestReplay.LapTime;
			...
			SpeedyRacerManager.Player.Update();
		...
		public void RenderLogos()
		{
			...
			if (Input.MouseInBox(websiteRect) &&
				Input.MouseLeftButtonJustPressed)
				Process.Start("http://www.SpeedyRacer.com");
#endif
			...
			string trackName, int[] top5LapTimes)
		{
			...
			if (SpeedyRacerManager.Player.GameOver)
			{
				speed = 0;
				gear = 1;
				acceleration = 0;
		...
		public void RenderTextsAndMouseCursor()
		{
			...
			if (Input.MouseDetected &&
				...
				SpeedyRacerManager.ShowMouseCursor)
			{
