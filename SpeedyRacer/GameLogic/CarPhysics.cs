// Project: SpeedyRacer, File: CarPhysics.cs
// Namespace: SpeedyRacer.GameLogic, Class: CarPhysics
// Path: C:\code\SpeedyRacer\GameLogic, Author: Abi
...
using SpeedyRacer.GameLogic.Physics;
...
namespace SpeedyRacer.GameLogic
{
	...
	public class CarPhysics : BasePlayer
	{
		...
		public const float DefaultMaxSpeed =
			...
			MaxPossibleSpeed =
			2* DefaultMaxSpeed;//290.0f * MphToMeterPerSec;
		...
		/// </summary>
		public const float DefaultMaxAccelerationPerSec = 2.5f,//5.0f,//120.0f,
			MaxAcceleration = 5.75f,//8.0f,//70.0f,//250.0f,
		...
		/// </summary>
		const float CarFrictionOnRoad = 17.523456789f;//15.0f;//2.5f;
		...
		/// </summary>
		const float AirFrictionPerSpeed = 0.66f;//0.225f;//0.33f;//0.42f;
		...
		public override void Update()
		{
			...
			if (SpeedyRacerManager.Player.FreeCamera)
				return;
			...
			if (isCarOnGround)
			{
				...
				{
					SpeedyRacerManager.Landscape.AddBrakeTrack(
			...
			SpeedyRacerManager.Landscape.UpdateCarTrackPosition(
			...
			if (trackSegmentNumber != oldTrackSegmentNumber &&
				// And we in game?
				SpeedyRacerManager.InGame)
			{
				// Was this the start? Did we finish a lap?
				if (trackSegmentNumber == 0 &&
					// Ignore if we missed one checkpoint.
					SpeedyRacerManager.Landscape.NewReplay.CheckpointTimes.Count >=
					SpeedyRacerManager.Landscape.CheckpointSegmentPositions.Count-1)
				{
				...
				else
				{
					...
					int num = SpeedyRacerManager.Landscape.NewReplay.CheckpointTimes.Count;
					if (num < SpeedyRacerManager.Landscape.CheckpointSegmentPositions.Count &&
						SpeedyRacerManager.Landscape.CheckpointSegmentPositions[num] >
						...
						trackSegmentNumber)
					{
						...
						int differenceMs =
							SpeedyRacerManager.Landscape.CompareCheckpointTime(num);
			...
			Matrix trackMatrix =
				SpeedyRacerManager.Landscape.GetTrackPositionMatrix(
				//SpeedyRacerManager.Player.CarTrackPosition,
			...
			SpeedyRacerManager.Player.SetGroundPlaneAndGuardRails(
				...
				(nextRoadWidth / 2 - GuardRail.InsideRoadDistance / 2) +
				trackMatrix.Forward);
			carRenderMatrix = SpeedyRacerManager.Player.UpdateCarMatrixAndCamera();
		...
		private void ApplyGravity()
		{
			...
			float distFromGround = Vector3Helper.SignedDistanceToPlane(
				...
				groundPlaneNormal);
			isCarOnGround = distFromGround > -2.5f;//-0.5f;//-0.1f;
			// Use very hard and instant gravity to fix if car is below ground!
			float maxGravity = Gravity * moveFactor * 5;// *2;// ;// *2;
			...
			{
				carPos.Z = (carPos.Z + groundPlanePos.Z)/2.0f;
			} // if
		} // ApplyGravity()
		#endregion
		...
		public Matrix UpdateCarMatrixAndCamera()
		{
			...
			if (SpeedyRacerManager.InMenu == false &&
				ZoomInTime > 1500)
			{
				...
				if (ZoomInTime-BaseGame.ElapsedTimeThisFrameInMilliseconds >= 3000)
					SpeedyRacerManager.Player.SetCameraPosition(camPos);
				else
					SpeedyRacerManager.Player.InterpolateCameraPosition(camPos);
			} // if (SpeedyRacerManager.InMenu)
			else if (SpeedyRacerManager.Player.FreeCamera)
				SpeedyRacerManager.Player.InterpolateCameraPosition(
					carPos + CarHeightVector +
					carMatrix.Forward * chaseCamDistance -
					carMatrix.Up * chaseCamDistance / (viewDistance+6.0f) -
					carMatrix.Up * 1.0f);
			else if (SpeedyRacerManager.InMenu &&
				BaseGame.TotalTimeMilliseconds < 100)
				// No interpolation in menu, just set it (at least for the first ms)
				SpeedyRacerManager.Player.SetCameraPosition(
					carPos + CarHeightVector +
					carMatrix.Forward * chaseCamDistance -
					//carMatrix.Up * chaseCamDistance / (viewDistance+6.0f) -
					carMatrix.Up * 0.6f);
			...
			if (SpeedyRacerManager.Player.GameTimeMilliseconds >
				SpeedyRacerManager.Landscape.NewReplay.NumberOfTrackMatrices *
				Replay.TrackMatrixIntervals * 1000.0f)
				SpeedyRacerManager.Landscape.NewReplay.AddCarMatrix(carMatrix);
		...
		public void DisplayPhysicsValuesAndHelp()
        {
			...
			new PlaneRenderer(
				...
				SpeedyRacerManager.Landscape.CityMaterial, 10).Render();
			new PlaneRenderer(
			...
			TextureFont.WriteText(2, 90,
				"Camera pos: " + SpeedyRacerManager.CameraPos);
			...
			TextureFont.WriteText(2, 420,
				"wannaCameraLookVector: " + SpeedyRacerManager.Player.wannaCameraLookVector);
			TextureFont.WriteText(2, 450,
				"wannaCameraDistance: " + SpeedyRacerManager.Player.wannaCameraDistance);
			 */
		...
		static public void TestCarPhysicsOnPlane()
		{
			...
			TestGame.Start("TestCarPhysicsOnPlane",
				delegate
				{
					...
					SpeedyRacerManager.Player.ZoomInTime = 0;
					SpeedyRacerManager.Player.SetCameraPosition(new Vector3(0, -5, 8));
				},
				delegate
				{
